...
class LlmCallContextTest {
    ...
    void shouldClearContext() {
        LlmCallContext.set("SCENARIO", 1L, 1L, 1L, 1L);
        LlmCallContext.clear();
        assertThat(LlmCallContext.get()).isEmpty();
    }
    ...
    void shouldSupportNullUserId() {
        LlmCallContext.set("ASYNC_TASK", null, 1L, 1L, null);
    ...
    @Test
    void shouldSupportNullProcessLogId() {
        LlmCallContext.set("TREE_SEARCH", null, null, null, null);

        var ctx = LlmCallContext.get();
        assertThat(ctx).isPresent();
        assertThat(ctx.get().scenario()).isEqualTo("TREE_SEARCH");
        assertThat(ctx.get().processLogId()).isNull();
    }

    @Test
    void shouldAllowOverwrite() {
        LlmCallContext.set("FIRST", 1L, 1L, 1L, 1L);
        LlmCallContext.set("SECOND", 2L, 2L, 2L, 2L);

        var ctx = LlmCallContext.get();
        assertThat(ctx.get().scenario()).isEqualTo("SECOND");
        assertThat(ctx.get().processLogId()).isEqualTo(2L);
    }
}
